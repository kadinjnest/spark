...
async function startVideoGeneration(
  ...
  if (!apiKey) return { taskId: null, error: "RUNWAY_API_KEY is not set" };
  ...
  try {
    ...
    if (!startRes.ok) {
      ...
      return { taskId: null, error: `Runway API error ${startRes.status}: ${errText}` };
    }
    ...
    if (!taskId) {
      ...
      return { taskId: null, error: `Runway returned no task id: ${JSON.stringify(startData)}` };
    }
...
export async function POST(req: NextRequest) {
  ...
  // Start video generation if an image was provided; polling happens client-side
  let taskId: string | undefined;
  let videoError: string | undefined;
  if (imageBase64) {
    const result = await startVideoGeneration(imageBase64, title, videoPrompt);
    taskId = result.taskId ?? undefined;
    videoError = result.error;
    if (videoError) console.error("Video generation start failed:", videoError);
  }
